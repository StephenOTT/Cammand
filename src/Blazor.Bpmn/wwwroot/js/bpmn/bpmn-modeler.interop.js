...
export function getAllElementPropertiesForCache(bpmnJsInstance) {
    return bpmnJsInstance.get("elementRegistry")._elements.map(el=>{
        return {
            id: el.id,
            type: el.type
        }
...
export function getRootElementId(bpmnJsInstance) {
    return bpmnJsInstance.get("canvas").getRootElement().id;
}
...
export function generateHeatmap(data, bpmnJsInstance, bpmnRef, heatmapInstance, maxValue) {
    ...
    document.querySelector(".heatmap-canvas").style.pointerEvents = "none"
...
export function setupViewboxChangingListener(bpmnJsInstance, dotNetRef) {
    console.log("creating listener for Viewbox changing")
    bpmnJsInstance.get("eventBus").on('canvas.viewbox.changing', function (context) {
            dotNetRef.invokeMethodAsync('ViewboxChangingEvent', "changing");
    });

    bpmnJsInstance.get("eventBus").on('canvas.viewbox.changed', function (context) {
        dotNetRef.invokeMethodAsync('ViewboxChangingEvent', "changed");
    });
}